	...
	int missing_data, int all)
{
	...
	// temporary variables
	double *U, *V, *zscore;	// beta, U, V
	double *C, *nC;		// covariable
	double alpha_R, *alpha_U, *alpha_V;
	...
	if (all) {
		...
		Kp = nD + K;
		U = (double *)calloc(Kp * N, sizeof(double));	// (N,K)
		V = (double *)calloc(Kp * M, sizeof(double));	// (N,K)
		alpha_V = (double *)calloc(Kp, sizeof(double));
		alpha_U = (double *)calloc(Kp, sizeof(double));

		printf("\n<<<<\n\t Analyse for all covariables.\n\n");
		// create C
		modify_U(U, nC, N, nD, nd, all);

		// run LFMM
		lfmm_emcmc(dat, I, zscore, U, V, alpha_U, &alpha_R,
			   alpha_V, N, M, K, D, epsilon, Niter, burn,
