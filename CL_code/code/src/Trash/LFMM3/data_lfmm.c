...
void zscore_calc(double *zscore, double *sum, double *sum2, int n, int cur, int D)
{
	...
	write_data_double("sum_3.txt",n*D, 1, sum);
	write_data_double("sum2_3.txt",n*D, 1, sum2);
	for (i = n; i < D * n; i++) {
		var =
...
void modify_U(double *U, double *C, int N, int nD, int d, int all)
{
	int i, nd;
...
double var_data(float *R, double *U, double *V, int N,
		int M, int K, double *thrd_m2, int num_thrd)
{
	double mean, mean2, tmp2, tmp;
	int i, j, k;
	/*
...
# ifndef WIN32
	if (num_thrd > 1) {
		thrd_var(R, U, V, NULL, NULL, K, 0, M, N, num_thrd, slice_var, 0, &mean,
				&mean2);
	} else {
# endif
		mean = 0.0;
		...
		for (i = 0; i < N; i++) {
			for (j = 0; j < M; j++) {
				...
				for (k = 0; k < K; k++)
					tmp += U[k * N + i] * V[k * M + j];
				tmp2 = ((double)(R[i * M + j]) - tmp);
				mean += tmp2;     //(double)(R[i*M+j])-tmp1-tmp2)
				mean2 += tmp2 * tmp2;  	//((double)(R[i*M+j])-tmp1-tmp2)*
...
double var_data_inputation(float *R, int *I, double *U, double *V,
	int N, int M, int K, double *thrd_m2, int num_thrd)
{
	double mean, mean2, tmp2, tmp;
	int i, j, k;
...
void inputation_lfmm(float *R, double *U, double *V, int *I,
		int N, int M, int K)
{
	int i, j, k;
	double tmp;
