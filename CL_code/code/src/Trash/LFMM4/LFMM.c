	...
	int missing_data, int all)
{
	...
	if (all) {
		beta = (double *)calloc(nD * M, sizeof(double));
		alpha_beta = (double *)calloc(nD, sizeof(double));
		perc_var = (double *)calloc(nD + K + 1, sizeof(double));
	} else {
	...
	normalize_mean_I_float(dat, N, M);
	...
	//	modify_C(nC, N, nD, C, nd, all);
		...
		// write zscore
	        write_zscore_double(output_file, M, zscore, D, 1, 0, K, N, dev, DIC, perc_var);
