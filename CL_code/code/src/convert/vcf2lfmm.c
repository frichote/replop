	...
	char* removed_bp_file)
{
	...
	allele = (int *)calloc((*M)*(*N),sizeof(int));
				...
				while(token)
				{
					...
					if (strlen(token) == 1) {
						...
						// else everything is fine
							...
							allele[j*(*N)+i] = (int)token[0] - (int)'0' + (int)token[2] - (int)'0';
						}
					}
					// next token
					token = strtok(NULL,SEP);
	...
	// look for last colum not removed
	jp = (*N);
	while (jp >= 0 && removed[jp] == 1) {
		jp--;
	}
	Np = jp;
	...
	for (jp = 0; jp<(*M); jp++) {
		...
		for (i = 0; i< Np-1; i++) {
			if (!removed[i]) {
				fprintf(genotype_File,"%d ",allele[jp*(*N)+i]);
				ip ++;
			} else {
				j ++;
			}
		}
		fprintf(genotype_File,"%d\n",allele[jp*(*N)+Np-1]);
	}
