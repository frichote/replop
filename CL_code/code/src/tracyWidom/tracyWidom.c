...
void tracyWidom(char* input_file, char *output_file)
{
	...
	// write output
	write_data_tracyWidom(output_file, N, values, pvalues, twstat, effectn, percentage);
	...
	double *pvalues, double *twstat, double *effectn, double *percentage)
{
	...
	// header
	fprintf(file,"N\teigenvalues\ttwstats\t\tpvalues\teffectn\tpercentage\n");
	for (i = 0; i < M; i++) {
		fprintf(file,"%d\t%3.4G\t\t%3.4G\t\t%3.4G\t\t%3.8G\t%3.4G\n", i+1, 
			values[i], twstat[i], pvalues[i], effectn[i], percentage[i]);
	}
	...
	int M) 
{
	...
	for (i = 0; i < M; i++) {
		...
		// mu
		mu = (sqrt(nhat-1) + sqrt(Mp)) * (sqrt(nhat-1) + sqrt(Mp)) / nhat; 
		// sigma
		sigma = (sqrt(nhat-1) + sqrt(Mp)) / nhat 
			* pow(1.0 / sqrt(nhat - 1) + 1.0 / sqrt(Mp), (double)(1.0/3.0));
		// lambda
...
double twtest(double lambda)
{
	...
	if (i == 161)
		...
		return (twtable[(i-1)*3+1] + (twtable[i*3+1]-twtable[(i-1)*3+1])
