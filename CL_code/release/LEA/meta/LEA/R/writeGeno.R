write.geno <- function(R, output_file) 
{

        if(missing(R))
                stop("'R' argument is missing.")
        else if (!(is.matrix(R) || is.data.frame(R) || is.vector(R)))
                stop("'R' argument has to be of type matrix, R.frame or vector.")
	else if (is.vector(R))
	...
	write.table(t(R), output_file, col.names=FALSE,row.names=FALSE,sep="");
