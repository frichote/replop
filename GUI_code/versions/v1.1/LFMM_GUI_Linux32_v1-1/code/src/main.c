...
int main(int argc, char *argv[])
{
	...
	// read of zscore2pvalue file
	z2p = (double *)calloc(2 * 2000, sizeof(double));	// (N,K)
	read_data_double2("./code/zscore2pvalue.txt", 2000, 2, z2p);
	...
	if (nd) {
		...
		lfmm_emcmc(dat, I, C, zscore, beta, U, V, alpha_beta, &alpha_R,
			   &alpha_U, N, M, K, D, z2p, epsilon, Niter, burn,
