	...
	if (newProjectDialog->exec() == QFileDialog::Accepted)
	{
		...
		if (project.data_prev.endsWith(tr(".ped")))
		{
			cmdLine =
				...
				.arg(newProjectDialog->dataLineEdit->text())
				.arg(newData)
				.arg(project.L)
				.arg(project.N);
		...
		{
			cmdLine =
				tr("cp %1 %2").arg(newProjectDialog->dataLineEdit->text()).arg(newData);
		}
#endif
...
#endif
		infoTextEdit->append(tr("%1, data conversion started and is running. Please wait...\n")
			.arg(CMD_TITLE));
		cle2->start(cmdLine);
		cle2->waitForFinished();
		infoTextEdit->append(tr("%1, data conversion ended without error.\n")
			.arg(CMD_TITLE));
#ifndef Q_WS_MAC
...
void LFMMGUI::cleFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
	...
	if (mainRun) // && !creationData && !creationRes)
	{
			   ...
			   QFile::exists(devName)) 
		{
			...
			{
				cmdLine =
					tr("perl scripts/pedsnp2zscore.pl %1 %2 %3 %4")
					.arg(zscoreName)
					.arg(fullNameData)
					.arg(fullNameRes)
					.arg(project.L);
			...
			QString fullNameData = project.path + "/" + project.nameData;
				cmdLine =
					tr("perl ./scripts/snp2zscore.pl %1 %2 %3 %4")
					.arg(zscoreName)
					.arg(fullNameData)
					.arg(fullNameRes)
					.arg(project.L);
			...
			QString fullNameData = project.path + "/" + project.nameData;
				cmdLine =
					tr("perl ./scripts/lfmmsnp2zscore.pl %1 %2 %3 %4")
					.arg(zscoreName)
					.arg(fullNameData)
					.arg(fullNameRes)
					.arg(project.L);
			...
			{
				cmdLine =
					tr("perl ./scripts/nothing2zscore.pl %1 %2 %3")
					.arg(zscoreName)
					.arg(fullNameRes)
					.arg(project.L);
			}
	   ...
	   if (applPath.contains(QChar(' ')))
	   {
	   	applPath = "\"" + applPath + "\""; 
			...
			this->setEnabled(false);
#endif
			infoTextEdit->append(tr("%1, the creation of zscore file started and is running. Please wait...\n")
				.arg(CMD_TITLE));
			cle2->start(cmdLine);
			cle2->waitForFinished();
			infoTextEdit->append(tr("%1, the creation of zscore file ended without error.\n")
				.arg(CMD_TITLE));
#ifndef Q_WS_MAC
			this->setEnabled(true);
#endif
...
void LFMMGUI::createGreenManhattan()
{
	...
	QString toHighlight= project.path + "/toHighlight_table.txt";
	QString plot= project.path + "/manhattan_plot.pdf";
	QString cmdLine = "Rscript scripts/manhattan.R " + table + " " + toHighlight + " " + plot; 
/*
#ifndef Q_WS_MAC
	this->setEnabled(false);
#endif
